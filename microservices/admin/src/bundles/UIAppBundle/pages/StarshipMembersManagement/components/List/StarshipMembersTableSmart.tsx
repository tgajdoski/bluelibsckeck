...
import { StarshipMemberList } from "../../config/StarshipMemberList";

export class StarshipMembersAntTableSmart extends AntTableSmart<StarshipMember> {
  collectionClass = StarshipMembersCollection;

  getBody(): QueryBodyType<StarshipMember> {
    return StarshipMemberList.getRequestBody();
  }

  getColumns(): ColumnsType<StarshipMember> {
    const list = this.container.get(StarshipMemberList);
    list.build();

    return [...list.rest(), this.getActionsColumn()];
  }

  getActionsColumn(): ColumnType<StarshipMember> {
    return {
  ...
  getSortMap() {
    return StarshipMemberList.getSortMap();
  }
  ...
  getActionItems() {
    ...
    if (features.view) {
      actions.push({
        ...
        icon: <EyeOutlined />,
        action: (model) => {
          this.router.go(Routes.STARSHIP_MEMBERS_VIEW, {
    ...
    if (features.edit) {
      actions.push({
        ...
        icon: <EditOutlined />,
        action: (model) => {
          this.router.go(Routes.STARSHIP_MEMBERS_EDIT, {
            params: { id: model._id.toString() },
          });
        },
      });
