...
import { TasksAntTableSmart } from "./TasksTableSmart";
...
export function TasksList() {
  ...
  const [api, Provider] = newSmart(TasksAntTableSmart);
  ...
  return (
    <UIComponents.AdminLayout>
      <Ant.PageHeader
        title={t("management.tasks.list.header")}
        extra={[
          features.create ? (
            <Ant.Button
              key="1"
              onClick={() => router.go(Routes.TASKS_CREATE)}
              icon={<PlusOutlined />}
            >
              {t("management.tasks.list.create_btn")}
            </Ant.Button>
          ) : null,
          <Ant.Button
            key="2"
      ...
      <Ant.Layout.Content>
        <Provider>
          <div className="page-tasks-list">
            {filtersOpened && <TasksListFilters onUpdate={onFiltersUpdate} />}
            <Ant.Input.Search
              name="Search"
