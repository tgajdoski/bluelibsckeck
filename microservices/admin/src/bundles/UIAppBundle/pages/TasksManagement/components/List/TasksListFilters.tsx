...
import { TaskListFiltersForm } from "../../config/TaskListFiltersForm";

type TasksListFiltersProps = {
  onUpdate: (filters: any) => void;
};

export const TasksListFilters = React.memo(TasksListFiltersBase);

export function TasksListFiltersBase(props: TasksListFiltersProps) {
  ...
  const filterForm = use(TaskListFiltersForm, { transient: true });
  filterForm.build();
