...
import { TaskViewer } from "../../config/TaskViewer";
import { features } from "../../config/features";

export function TasksView(props: { id: string }) {
  ...
  if (isLoading) {
    ...
    if (error || document === null) {
      ...
      content = <TasksViewComponent document={document} />;
    }
  }
  ...
  return (
    <UIComponents.AdminLayout>
      <Ant.PageHeader
        title={t("management.tasks.view.header")}
...
export function TasksViewComponent(props: { document: Partial<Task> }) {
  const document = props.document;

  const viewer = use(TaskViewer, { transient: true });
...
export function getHeaderActions(
  ...
  if (features.edit) {
    actions.push(
      <Link
        key="edit"
        to={router.path(Routes.TASKS_EDIT, {
          params: { id },
        })}
      >
      ...
      <Ant.Popconfirm
        ...
        onConfirm={() => {
          collection.deleteOne(id).then(() => {
            router.go(Routes.TASKS_LIST);
            Ant.notification.success({
              message: "Success",
              description: "You have deleted the Task",
            });
          });
        }}
      >
  ...
  const viewRoutePath = router.path(Routes.TASKS_VIEW, {
    params: { id },
  });
